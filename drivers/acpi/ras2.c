...
static void ras2_aux_dev_idx_free(int idx)
{
	ida_free(&ras2_ida, idx);
}

static void ras2_remove_aux(struct auxiliary_device *auxdev)
{
...
static void ras2_release(struct device *device)
{
	...
	ras2_remove_aux(auxdev);
	ras2_aux_dev_idx_free(auxdev->id);
