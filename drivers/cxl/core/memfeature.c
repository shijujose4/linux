				 ...
				 int fru_id, struct cxl_ecs_params *params)
{
	...
	params->threshold = 0;
	params->count_mode = 0;
	data_size = cxl_get_feature(&cxlmd->cxlds->cxl_mbox, CXL_FEAT_ECS_UUID,
				    CXL_GET_FEAT_SEL_CURRENT_VALUE,
				    rd_attrs, rd_data_size);
	if (!data_size)
		return -EIO;
				 ...
				 u8 param_type)
{
	...
	ret = cxl_set_feature(&cxlmd->cxlds->cxl_mbox, CXL_FEAT_ECS_UUID,
				 ...
				 struct cxl_memdev_ppr_params *params)
{
	...
	data_size = cxl_get_feature(&cxlmd->cxlds->cxl_mbox, cxl_ppr_ctx->repair_uuid,
				    CXL_GET_FEAT_SEL_CURRENT_VALUE,
				    rd_attrs, rd_data_size);
	if (!data_size)
		return -EIO;
				     ...
				     struct cxl_memdev_sparing_params *params)
{
	...
	data_size = cxl_get_feature(&cxlmd->cxlds->cxl_mbox,
				 ...
				 struct edac_dev_feature *ras_feature, u8 scrub_inst)
{
	...
	if (cxlr) {
		...
		for (i = p->interleave_ways - 1; i >= 0; i--) {
			...
			idx = cxl_get_supported_feature_entry(cxl_mbox,
							      &CXL_FEAT_PATROL_SCRUB_UUID);
			if (idx < 0)
				goto feat_unsupported;

			feat_entry = &cxl_mbox->features->entries[idx];
		...
		cxlds = cxlmd->cxlds;
		cxl_mbox = &cxlds->cxl_mbox;
		idx = cxl_get_supported_feature_entry(cxl_mbox, &CXL_FEAT_PATROL_SCRUB_UUID);
		if (idx < 0)
			goto feat_unsupported;
			       ...
			       struct edac_dev_feature *ras_feature)
{
	...
	idx = cxl_get_supported_feature_entry(cxl_mbox, &CXL_FEAT_ECS_UUID);
	if (idx < 0)
		goto feat_unsupported;
				    ...
				    u8 repair_inst)
{
	...
	idx = cxl_get_supported_feature_entry(cxl_mbox, &CXL_FEAT_SPPR_UUID);
	if (idx < 0)
		goto feat_unsupported;
				   ...
				   u8 repair_inst)
{
	...
	idx = cxl_get_supported_feature_entry(cxl_mbox, &desc->repair_uuid);
	if (idx < 0)
		goto feat_unsupported;
